...
/*
 * Puts the defines onto the Lighting5 function
 * Allows access to the message types through
 * Lighting5.LIGHTWAVERF etc.
 */
for (var k in defines.lighting5) {
  Lighting5[k] = defines.lighting5[k];
}
...
Lighting5.prototype._sendCommand = function (deviceId, command, level, callback) {
  var self = this,
      ...
      level = level || 0x1f,
      buffer = [0x0a, defines.messageTypes.LIGHTING5, self.subtype, cmdId,
...
Lighting5.prototype.switchOn = function (deviceId, options, callback) {
  ...
  if (typeof options.level !== "undefined") {
    if (options.level < 1 || options.level > 31) {
      throw new Error("Invalid level value must be in range 1-31.");
      }
  };
  var command = options.command || Lighting5.ON,
