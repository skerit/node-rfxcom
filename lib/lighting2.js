...
/*
 * Puts the defines onto the Lighting2 function
 * Allows access to the message types through
 * Lighting2.ANSLUT etc.
 */
for (var k in defines.lighting2) {
  Lighting2[k] = defines.lighting2[k];
}
...
Lighting2.prototype._sendCommand = function (deviceId, command, level, callback) {
  var self = this,
      ...
      level = level || 0xf,
      buffer = [0x0b, defines.messageTypes.LIGHTING2, self.subtype, cmdId, device.idBytes[0],
